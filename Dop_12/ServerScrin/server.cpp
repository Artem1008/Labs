...
DWORD Server::ConnectToClient(LPVOID client_socket)
{
    SOCKET my_sock;
...
void Server::GetBitmap()
{
    SOCKET my_sock;
    my_sock = ((SOCKET *)client_socket)[0];
    DWORD dwSize;
    recv(my_sock, (char*)&dwSize, sizeof(DWORD), 0);

    // Выделяем память
    BYTE* pBits = new BYTE[dwSize];
    recv(my_sock, (char*)pBits, dwSize, 0);

    // Создаем совместимый HBITMAP
    HDC hdc = GetDC(NULL);
...
int Server::StartServer()
{
    if (listen(server_sock, 0x100))
        ...
        while ((client_socket = accept(server_sock, (sockaddr *)&client_addr, &client_addr_size)))
        {
            ...
            CreateThread(NULL, NULL, ConnectToClient, &client_socket, NULL, &thID);
        }
        return 0;
}
