...
void OutputStr(const string sStudentName, const string sGroupName)
{
    stringstream ss;
    //формируем заголовок
    ss << "*****" << setw(SPACING) << "ТУСУР" << setw(SPACING) << "*****\n";
    //вычисляем его длину
    ...
    ss << setw((LengthFirstStr-FinishStr.length())/2+FinishStr.length()) << FinishStr+"\n\n";
    cout << ss.str();
}
void OutputHeading()
{
    cout << "Введите номер требуемого вычисления:\n";
...
void CalcSpeed()
{
    ...
    cout << "Скорость прямолинейного движения: " << Interval*Time << "\n";
}
void CalcBoost()
{
    ...
    cout << "Введите время:\n";
    if(ChekValue(Time) == -1) return;
    cout << "Ускорение: " << (EndSpeed-BeginSpeed)/Time << "\n";
}
void CalcRadius()
{
    float Circumference;
    ...
    cout << "Радиус круга: " << Circumference/(2*M_PI) << "\n";
}
template<typename T>
int ChekValue(T &Value)
{
    ...
    cout<<"Привышено число попвток ввода данных\n";
    return -1;
}
