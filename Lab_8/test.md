...
| Можно ли добавлять новые элементы? Если да, то каким образом? С какой стороны разрешено их добавлять? | Нельзя.| Нельзя. | push_back() добавляет элемент в конец вектора, insert(n,data) добавляет элемент в произвольное местосо сдвигом |  push_back() добавляет элемент в конец вектора, insert(n,data) добавляет элемент в произвольное местосо сдвигом | 
| Можно ли удалять элементы? Если да, то каким образом? С какой стороны разрешено их удалять?| Нельзя. | Нельзя. | erase удаляет произвольный элемент, clear очищает контейнер , pop_back удаляет последний элемент,
pop_frontудаляет первый элемент; | erase удаляет произвольный элемент |
